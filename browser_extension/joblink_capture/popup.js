...
async function capturePage() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(payload.error || 'Could not send this page to JobLink.');
    }
    ...
    const warning = Number(debug.text_length || 0) < 50
      ...
      : '';
    setStatus(`Captured: ${job.job_title || 'job page'}.${warning} Return to JobLink and select Load captured jobs.`, 'success');
  } catch (error) {
