...
Thanks for trying JobLink. This is a small local beta, so the goal is not to prove that every result is perfect. The useful part is finding the pages where a field is missing, messy, or too confidently wrong.

## What You Need

- The current `JobLink-Tracker-Windows.zip` from this repository.
...
1. Extract the ZIP into a new folder and open `JobLink Tracker.exe`.
...
9. Close the JobLink window, open it again, and confirm the app starts normally.
...
```text
%LOCALAPPDATA%\JobLink Tracker\logs\user_reported_issues.jsonl
%LOCALAPPDATA%\JobLink Tracker\logs\beta_feedback.jsonl
```

Send only those files after checking that you are comfortable sharing their contents. A useful report says which website was tested, which field was wrong, what JobLink returned, and what the posting showed instead.
...
> Hi, I am testing a local job-tracker tool called JobLink. It turns job-posting links into reviewed Excel rows. The test takes about 20 minutes, does not require Python, and keeps files on your computer. Please try 10 to 15 current postings from a few websites and tell me where the company, title, location, work type, salary, or Excel update is wrong.
