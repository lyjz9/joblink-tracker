...
For a short, repeatable test session and a message you can send to testers, use the [beta test guide](beta_testing.md).

## What Testers Download

The build creates `Linc-v0.1.0-Windows.zip`. Extract it, open the `Linc` folder, and double-click `Linc.exe`.

The executable:

1. Starts the local app at `http://127.0.0.1:5050`.
2. Opens Linc in the default browser.
3. Shows a small window with **Open Linc** and **Stop** buttons.
4. Stops Linc when that window closes.

Linc always uses port `5050` so the Chrome capture extension knows where to send a page. If another program already uses that port, the launcher explains the conflict and stops.
...
2. Select **Build Linc Windows beta**.
3. Select **Run workflow**.
4. Wait for the Windows build to finish.
5. Download the `Linc-v0.1.0-Windows` artifact.
...
The build sets `PLAYWRIGHT_BROWSERS_PATH=0`, Playwright's supported way to bundle Chromium with PyInstaller. It also uses `--only-shell` because Linc runs Chromium in the background and does not need the second visible-browser executable.

Build output is written to:

```text
dist\Linc\
dist\Linc-v0.1.0-Windows.zip
```
...
Only use a ZIP downloaded from this GitHub repository. On the SmartScreen prompt, select **More info**, check that the file is `Linc.exe`, and then choose **Run anyway**. Do not bypass the warning for a copy from an email, chat, or another download site.
...
2. Start `Linc.exe` without Python active.
