...
Linc is still a beta, and no scraper can read every job site perfectly. Company career pages and ATS links usually give the best results, especially Greenhouse, Lever, Ashby, Workday, iCIMS, Breezy, and SmartRecruiters.

Some job boards hide details behind login walls, Cloudflare checks, human verification, private APIs, or scripts that load after the page opens. When that happens, Linc may return an error or only part of the posting. Check every row marked `Review` before saving it.
