...
- Both parsers return the fields used by the tracker: `date_applied`, `company`, `job_title`, `job_link`, `status`, `location`, `work_type`, `salary`, `follow_up`, and `application_portal`. The internal `source` field remains as a compatibility alias for `application_portal` while older sessions and workbooks migrate.
