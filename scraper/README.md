...
Linc tries the browser scraper first and falls back to a lighter HTML parser when needed.
...
- Both parsers return the fields used by the tracker: `date_applied`, `company`, `job_title`, `job_link`, `status`, `location`, `work_type`, `salary`, `follow_up`, `found_on`, and `application_portal`. The internal `source` field remains as a compatibility alias for `application_portal` while older sessions and workbooks migrate.
